...
public class MIDTLZ3 extends ObjectiveFunction {

    DTLZ dtlz;
    List<List<Double>> paretoHistory = new ArrayList<>();

    public MIDTLZ3(int K, int M){
    ...
    public Double call() {
        ...
        for(int i=0; i<dtlz.M; i++){
            fs.add(Math.pow(dtlz.calculateCosSinProduct(X.getSubVector(0, dtlz.M-i).mapMultiplyToSelf(1.570))  * (1 + g),4));
        }
