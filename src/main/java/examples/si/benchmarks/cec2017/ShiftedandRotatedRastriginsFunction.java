...
public class ShiftedandRotatedRastriginsFunction extends ObjectiveFunction {

    private double fBias = 400.0;
    private RealMatrix fShift;
    private RealMatrix fMatrix;

    public ShiftedandRotatedRastriginsFunction() {}
    @Override
    public ObjectiveFunction updateDimensions(int n){
        this.numberOfDimensions = 30;
        fShift = new FileHandler("src/main/java/examples/si/benchmarks/cecdata/cec2017/shift_data_4.txt").read().toRealMatrix().getSubMatrix(0,0, 0, 29);
        fMatrix = new FileHandler("src/main/java/examples/si/benchmarks/cecdata/cec2017/M_4_D30.txt").read().toRealMatrix();
