...
public class RotatedHighConditionedEllipticFunction extends ObjectiveFunction {

    private double fBias = 100.0;
    private RealMatrix fShift;
    private RealMatrix fMatrix;

    public RotatedHighConditionedEllipticFunction() {}

    @Override
    public ObjectiveFunction updateDimensions(int n){
        this.numberOfDimensions = 30;
        fShift = new FileHandler("src/main/java/examples/si/benchmarks/cecdata/cec2014/shift_data_1.txt").read().toRealMatrix().getSubMatrix(0,0, 0, 29);
        fMatrix = new FileHandler("src/main/java/examples/si/benchmarks/cecdata/cec2014/M_1_D30.txt").read().toRealMatrix();
